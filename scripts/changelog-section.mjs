...
// release body.
//
// The app renders that body back through SimpleMarkdown in the update sheet and
// the What's New sheet, so the release notes and the changelog can never drift:
// there is one source, sliced at release time. Everything above the first
// `## [x.y.z]` heading (the file's preamble and the format guide) and the
// link-reference footer are dropped.
//
//   node scripts/changelog-section.mjs 1.7.0 [path/to/CHANGELOG.md]
//
...
// Matches `## [1.7.0] - 2026-08-18`, `## 1.7.0`, `## v1.7.0 – date`.
...
if (start === -1) {
  console.error(
    `No "## [${version}]" section in ${path}.\n` +
      `Add one (move the [Unreleased] entries under it) before releasing.`,
  );
  process.exit(1);
}
