...
export function useNotifications() {
  return useQuery<NotificationDto[], Error>({
    ...
    queryFn: () => notificationsClient.getAll(),
    staleTime: 1 * 60 * 1000,   // 1 minute — notifications are more time-sensitive
    gcTime: 5 * 60 * 1000,
    retry: 1,
    refetchOnWindowFocus: false,
    // TODO: API — add pagination support when backend adds GET /api/Notifications?page=&limit=
  })
}
...
// response shape is not yet typed. We cast to number and default to 0.
